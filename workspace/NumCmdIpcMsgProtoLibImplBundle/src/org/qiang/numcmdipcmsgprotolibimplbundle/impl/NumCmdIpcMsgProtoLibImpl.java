...
public class NumCmdIpcMsgProtoLibImpl implements IpcMsgProtoLib {
	...
	public byte[] cmd_ipc_lib( int dev_id, String cmd, byte[] data_byte, int data_len ) throws Exception {
		...
		msg_send_buf[msg_type_pos + 1] = (byte)0x0;
		msg_send_buf[msg_type_pos] = ConfigMsgType;
		
		msg_send_buf[msg_len_pos + 1] = (byte)( ( ( send_len - ipc_frm_hdr_size ) >> 8 ) & 0xff );
		msg_send_buf[msg_len_pos] = (byte)( ( send_len - ipc_frm_hdr_size ) & 0xff );
		
		msg_send_buf[command_type_pos + 1] = cmd_byte[0];
		msg_send_buf[command_type_pos] = cmd_byte[1];
		
		msg_send_buf[corr_tag_pos + 1] = (byte)( ( corr_tag_data >> 8 ) & 0xff );
		msg_send_buf[corr_tag_pos] = (byte)( corr_tag_data & 0xff );
		
		msg_send_buf[data_len_pos + 1] = (byte)( ( data_len >> 8 ) & 0xff );
		msg_send_buf[data_len_pos] = (byte)( data_len & 0xff );
		...
		for ( i = 0; i < data_len; i++ ) {
			return_byte[i] = recv_byte[data_pos + i];
		}
